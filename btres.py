...
linhaArquivo = '' # para retorno do valor
...
class BTree(object):
    ...
    def search(self, value, node=None):
        ...
        if value in node.keys:
            ...
            # node.keys[i] valor do nó que esta na folha
            # value = valor da linha buscado
            # If we are in a leaf, there is no more to check.
            if value <= node.keys[i]:
                linhaInicialBuscada = node.keys[i] * (int(value/node.keys[i]))
                contaLinhas = 0
                for linha in pd.read_csv(path, chunksize = node.keys[i], skiprows= linhaInicialBuscada - 1):
                    contaLinhas += 1
                    if contaLinhas == value:
                        linhaArquivo = linha
                        break
            return False
        else:
...
# retorna lista de indices do arquivo de entrada
def criaVetorNos(path):
    indices = []
    tamanho_bloco = 100000 # linhas por vez
    indice_final_bloco = 0

    for bloco in pd.read_csv(path, chunksize=tamanho_bloco):
        indice_final_bloco += tamanho_bloco
        indices.append(indice_final_bloco)
    
    return indices

# TODO: buscar no arquivo real os indices com criaVetorNos; colocar eles na arvore ; testar se consigo retornar
